...
def load_data():
    ...
    return df[["Country", "Item", "Value"]].copy()
...
def train_model(df):
    ...
    return pipeline, X_train, X_test, mae, r2


pipeline, X_train, X_test, mae, r2 = train_model(df)

# --------------------------------------------------
# SHAP explainer (NO caching to avoid hashing errors)
# --------------------------------------------------
...
with st.expander(
    " About this project (summary, data, methodology & conclusions)",
    expanded=True,
):
    st.markdown(
        """
###  Project context
This dashboard is a **demonstration data science case study** inspired by analytical
...
Performance metrics (MAE, R²) are shown for transparency.

###  Explainability (SHAP)
SHAP explains each estimate by decomposing it into feature contributions:
- Positive SHAP values increase the estimate
- Negative SHAP values decrease it

Aggregated SHAP values enable **cross-country structural comparison**.

###  Key findings (in a nutshell)
- **Item type** is the dominant driver of production magnitude
- **Country effects** are present but secondary
- Countries differ in how strongly item choice influences output

###  Conclusion
Explainable machine learning can complement traditional statistical analysis
by revealing **structural drivers** behind observed agricultural production patterns.
"""
    )
...
tab1, tab2, tab3 = st.tabs(
    ["Explore", "Predict + Explain", "Insights"]
)
...
with tab1:
    ...
    st.plotly_chart(
        px.bar(top_items, x="Item", y="Value"),
        use_container_width=True,
    )

    st.download_button(
        "⬇️ Download country data (CSV)",
...
with tab2:
    st.caption(
        "Select a country and item to view a model estimate and SHAP explanation."
    )

    country = st.selectbox(
        "Country", sorted(df["Country"].unique()), key="p_country"
    )
    item = st.selectbox(
        "Item",
        sorted(df[df["Country"] == country]["Item"].unique()),
        key="p_item",
    )
    ...
    shap_vals = explainer.shap_values(
        X_trans, check_additivity=False
    )[0]
    ...
    st.download_button(
        ...
        file_name="shap_contributors.csv",
    )

# ------------------- Insights ---------------------
with tab3:
    st.caption(
        "Aggregated SHAP metrics to compare structural drivers across countries."
    )

    X_sample = X_test.sample(
        min(30, len(X_test)), random_state=42
    )

    X_trans, names = transform_with_names(
        pipeline, X_sample
    )
    ...
    summary = (
        ...
        .apply(
            lambda g: pd.Series(
                {
                    "avg_abs_country_effect": g[country_cols]
                    .abs()
                    .mean()
                    .mean(),
                    "avg_abs_item_effect": g[item_cols]
                    .abs()
                    .mean()
                    .mean(),
                }
